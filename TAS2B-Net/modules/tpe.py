...
class TemporalPositionalEncoding(nn.Module):
    ...
    def __init__(
        ...
        if pos_enc_type == 'fourier':
            self.sin_table = None
            self.pe = LearnableFourierPositionalEncoding(m=1, f=d_model, h=32, d=d_model)
        elif pos_enc_type == 'rnn':
            sin_table = get_positional_encoding(d_model, max_len=max_len, T=T, scale=scale)
            self.sin_table = nn.Embedding.from_pretrained(sin_table.float(), freeze=True, padding_idx=0)
    ...
    def forward(
            self,
            temp_idx: Tensor,   # date_pos
            gdd: Optional[Tensor] = None,
            key_padding_mask: Optional[Tensor] = None,  # temporal_mask
